...
export function getPerformanceProfile(): PerformanceProfile {
  ...
  // ── Tier selection ────────────────────────────────────────────────────
  // LITE: genuinely weak hardware only. NOTE: Chrome caps navigator.deviceMemory
  //   at 8, so a value of 8 means "8GB OR MORE" — `mem <= 8` would classify
  //   EVERY Chrome machine (including 64GB workstations) as lite, silently
  //   disabling crop-zoom refinement for everyone. Only ≤4 is a reliable
  //   low-RAM signal. An integrated GPU alone is also not enough to downgrade
  //   (a 22-thread Core Ultra with Arc graphics runs the balanced CPU path
  //   comfortably), so the iGPU rule additionally requires a low core count.
  // MAX:  workstations with discrete GPU AND 24GB+ RAM AND 12+ cores.
  // BALANCED: everything else.
  const looksLite =
    (mem !== null && mem <= 4) ||
    cores <= 4 ||
    (isIntegratedGpu && cores <= 8)

  // NOTE: deviceMemory is capped at 8 by Chrome, so a `mem >= 24` requirement
  // could NEVER be satisfied there — max tier was unreachable in Chrome on any
  // hardware. High core count is the reliable strong-machine signal instead.
  // BUT high core count alone must NOT promote an integrated-GPU laptop to max:
  // a 22-thread Core Ultra with Arc graphics has the cores but not the GPU/RAM
  // headroom for max's 56 Hz / 1920px / 60 ms-refine cadence — on an iGPU with
  // little free RAM that just trips FrameBudget backoff. Max via core count now
  // additionally requires a non-integrated GPU; discrete GPUs still reach max.
  const looksMax =
    (isDiscreteGpu || (cores >= 16 && !isIntegratedGpu)) &&
    cores >= 12

  // Manual override for when the heuristic gets the hardware wrong:
  // ?perfTier=lite|balanced|max in the URL, or localStorage.musashiPerfTier.
  let forcedTier: PerformanceTier | null = null
  try {
    const fromUrl = new URLSearchParams(window.location.search).get('perfTier')
    const fromStorage = window.localStorage?.getItem('musashiPerfTier')
    const candidate = (fromUrl || fromStorage || '').toLowerCase()
    if (candidate === 'lite' || candidate === 'balanced' || candidate === 'max') {
  ...
  const tier: PerformanceTier = forcedTier ?? (looksLite ? 'lite' : looksMax ? 'max' : 'balanced')
  ...
  const profile: PerformanceProfile = {
    ...
    reason: forcedTier
      ? `${forcedTier} (forced via perfTier override)`
      : looksLite
