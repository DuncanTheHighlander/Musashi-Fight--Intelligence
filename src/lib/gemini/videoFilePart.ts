/**
 * Gemini video parts — Files API URI or inline bytes — with configurable FPS
 * and optional analysis window (startOffset/endOffset).
 *
 * FPS policy (vision-first rebuild):
 *   - The Gemini API accepts 0 < videoMetadata.fps <= 24 (verified 2026-07-23,
 *     https://ai.google.dev/gemini-api/docs/video-understanding).
 *   - The main evidence pass samples at MUSASHI_GEMINI_VIDEO_FPS (default 24).
 *   - FPS is configurable, never hard-coded at call sites; explicit `fps`
 *     options still win for benchmarks (12 vs 24) and diagnostics.
 *
 ...
 * normalized analysis artifact is under this size. The normalizer targets
 * ≤1280-wide/30fps so a ≤30s window stays well under this ceiling.
 */
export const MAX_INLINE_VIDEO_BYTES = 20 * 1024 * 1024
export const MAX_INLINE_VIDEO_LABEL = '20 MB'

/** Gemini API hard ceiling for videoMetadata.fps (docs: 0 < fps <= 24). */
export const GEMINI_FPS_MAX = 24

/** Default sampling rate for the vision evidence pass. */
export const GEMINI_FPS_DEFAULT = 24
...
/** Clamp an fps candidate into Gemini's accepted range (0, 24]. */
export function clampGeminiFps(fps: number): number {
  if (!Number.isFinite(fps) || fps <= 0) return GEMINI_FPS_DEFAULT
  return Math.min(fps, GEMINI_FPS_MAX)
}

/**
 * Configured sampling FPS for Gemini video parts.
 * MUSASHI_GEMINI_VIDEO_FPS overrides (clamped to (0, 24]); default 24.
 */
export function geminiConfiguredVideoFps(): number {
  const raw = Number(process.env.MUSASHI_GEMINI_VIDEO_FPS)
  if (Number.isFinite(raw) && raw > 0) return clampGeminiFps(raw)
  return GEMINI_FPS_DEFAULT
}

/**
 * FPS for a sport. Retained for call-site compatibility: since the vision-first
 * rebuild every sport samples at the configured evidence FPS (default 24) —
 * the old striking-10 / grappling-5 split cost accuracy on fast exchanges and
 * the coaching pass no longer re-sends video, so one well-sampled pass wins.
 */
export function geminiVideoFpsForSport(_sport?: string | null): number {
  return geminiConfiguredVideoFps()
}
...
export type GeminiVideoPartOptions = {
  ...
  /** Sport is accepted for compatibility; FPS no longer varies by sport. */
  sport?: string | null
  /** Explicit fps override; wins over the configured default when finite. */
  fps?: number | null
  /**
...
/** Inline-bytes part for the evidence / chat pass when file < 20MB. */
